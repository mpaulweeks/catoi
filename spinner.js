...
var speeds = [0];
for(var i = 0; i < positiveSpeeds.length; i++){
  speeds.unshift(positiveSpeeds[i]);
  speeds.push(positiveSpeeds[i]);
}
var index = parseInt((speeds.length/2));
var music;

function updateSpeed(){ 
  var secs = speeds[index];
  var reverse = (index < speeds.length/2) ? ' reverse' : '';
  
  $('.catbox').html('<img class="catoi"'
  + ' src="catoi.png"'
  + ' style="'
